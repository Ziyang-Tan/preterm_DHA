...
testTree <- function(phylo,
  ...
  for( i in seq_len(nrow(td))[-which(td$node == td$parent)]){
    ...
    if (sig_test=="ttest") { 
      test_total <- tryCatch(
        {t.test(formula=as.formula(paste0("`perc_total_",cell_type, "`~class")),
                data=prop_df)
        },
        error=function(e){
          message(e)
      ...
      test_parent <- tryCatch(
        {t.test(formula=as.formula(paste0("`perc_parent_1_",cell_type, "`~class")),
                data=prop_df)
        },
        error=function(e){
          message(e)
